...
Registers all module routers (via app.api.router's aggregation) and
global exception handlers. This is the single entry point for the
backend.

Run with:
    uvicorn app.main:app --reload

No database lifespan hook here (yet): Skill Matching, the only module
with real code on this branch as of Milestone 12, persists nothing. Once
a module that needs a database is integrated, its lifespan setup is a
small, additive change here — inventing one now for a module that
doesn't need it would be exactly the kind of premature complexity this
whole project has been disciplined about avoiding.
"""
